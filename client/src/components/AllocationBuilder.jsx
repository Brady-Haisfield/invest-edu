    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: 10, color: 'var(--text-muted)', lineHeight: 1.5, margin: 0 }}>
          Return methodology: Individual stocks use a weighted blend of Wall Street analyst consensus price targets (40%), Bogle fundamental model using forward EPS estimates (35%), and CAPM risk-adjusted market return (25%), adjusted for recent news sentiment. ETFs use CAPM with actual beta, blended with dividend yield and (when available) holdings-weighted analyst targets. Bond ETFs use live Federal Reserve Treasury yields plus a duration-based rate-sensitivity adjustment. REITs use dividend yield weighted with FFO growth proxy and yield spread vs. Treasuries. CAPM's risk-free rate and market-return inputs are both live (10-year Treasury yield; S&P 500 forward return derived from the current Shiller CAPE ratio) rather than fixed historical assumptions. Pessimistic/optimistic ranges are generated via Monte Carlo simulation using each security's estimated volatility (from beta, plus company-specific risk for individual stocks/REITs, or bond duration) — not a fixed percentage spread. All data updated on every search. This is a modeled estimate, not a guarantee of future returns.
        </p>
